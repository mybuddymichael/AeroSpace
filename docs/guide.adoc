...
Width of a single tiled window in a workspace is configurable via `single-window-width-percent`.
The option supports per-monitor configuration: `single-window-width-percent = [{ monitor.main = 70 }, 100]`.
